...
const distDir = path.resolve(__dirname, '../node_modules/sequelize');
...
const Support = {
  ...
  getSupportedDialects() {
    return fs
      .readdirSync(path.join(distDir, 'lib/dialects'))
  ...
  expectsql(query, assertions) {
      ...
      if (expectations.default !== undefined) {
        ...
        throw new Error(
          `Undefined expectation for "${
            Support.sequelize.dialect.name
    ...
    if (assertions.bind) {
      const bind =
        assertions.bind[Support.sequelize.dialect.name] ||
